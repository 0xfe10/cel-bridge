...
```toml
[dependencies]
cel-bridge = { git = "https://github.com/0xfe10/cel-bridge.git", tag = "v0.7.1", package = "cel-bridge" }
serde_json = "1.0"
```
...
The default build downloads the exact `v0.7.1` runtime artifact for Cargo's
...
https://github.com/0xfe10/cel-bridge/releases/download/
  v0.7.1/cel-bridge-manifest-v0.7.1.json
```
...
The build script appends `/v0.7.1/<file>`. HTTPS is required. HTTP is accepted
...
Use the shared `cel-bridge-android-*-v0.7.1.tar.gz` runtime artifacts or build
...
`cel-bridge-ios-xcframework-v0.7.1.zip` contains device and universal simulator
...
artifact is `cel-bridge-windows-x86_64-v0.7.1.zip`. Both files must match
