...
desktop, Android, iOS, or Web applications. It targets the current `v0.5.0`
API.
...
Windows ARM64 is not included in `v0.5.0`. Linux AArch64 is supported by the
...
Future<void> main() async {
  try {
    ...
    final batch = await runtime.evaluateMany(
      environment: environment,
      sources: ['age >= 18', 'country == "CN"'],
      variables: {'age': 20, 'country': 'CN'},
    );
    for (final item in batch) {
      switch (item) {
        case CelBatchSuccess(:final value):
          print(value);
        case CelBatchFailure(:final error):
          print('${error.code}: ${error.message}');
      }
    }

    final requests = await runtime.evaluateRequests(
      environment: environment,
      requests: const [
...
hooks:
  user_defines:
    cel_bridge:
      build_from_source: false
      release_base_url: "https://artifacts.example.com/cel-bridge/v0.5.0"
```
...
```text
cel-bridge-manifest-v0.5.0.json
cel-bridge-linux-x86_64-dynamic-v0.5.0.tar.gz
cel-bridge-android-arm64-v8a-v0.5.0.tar.gz
...
```
...
- the script phase downloads and verifies
  `cel-bridge-ios-xcframework-v0.5.0.zip` from the Release;
...
```bash
export CEL_BRIDGE_IOS_XCFRAMEWORK_URL="https://artifacts.example.com/cel-bridge/v0.5.0/cel-bridge-ios-xcframework-v0.5.0.zip"
export CEL_BRIDGE_IOS_XCFRAMEWORK_CHECKSUM_URL="https://artifacts.example.com/cel-bridge/v0.5.0/checksums.txt"
