...
func TestDecodeNestedTypes(t *testing.T) {
	...
	if len(value.Variables) != 4 || value.Variables["tags"].Element.Name != "string" {
...
func TestDecodeRejectsInvalidEnvironment(t *testing.T) {
	cases := []string{
		...
		`{"schemaVersion":1,"variables":{"values":{"type":"map","key":{"type":"double"},"value":{"type":"string"}}}}`,
