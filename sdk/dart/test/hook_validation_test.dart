...
void main() {
  test('accepts only the expected release artifact filename', () {
    expect(
      validateReleaseArtifactFile(
        file: 'cel-bridge-linux-x86_64-dynamic-v0.3.0.tar.gz',
  ...
  test('rejects path traversal and absolute artifact filenames', () {
    for (final file in [
      '../pubspec.yaml',
      '/tmp/cel-bridge-linux-x86_64-dynamic-v0.3.0.tar.gz',
      r'..\pubspec.yaml',
      'cel-bridge-linux-x86_64-dynamic-v0.3.0.tar.gz/../../pubspec.yaml',
    ]) {
      expect(
        () => validateReleaseArtifactFile(
          file: file,
